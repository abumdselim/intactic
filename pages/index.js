          ...
          <div className="section-container">
            <div className="caps-header">
              <div>
                ...
                <h2 className="section-title">Drive Growth with Our Core<br />Capabilities</h2>
              </div>
              <a href="#contact" className="btn-outline">Explore All Services <i className="fa-solid fa-arrow-right-long"></i></a>
            </div>
            <div className="caps-grid">
              {[
                { icon: 'fa-solid fa-people-group', title: 'Staff Augmentation', desc: 'Instantly scale your team with vetted AI-cloud experts. Pay only for the talent you need, when you need it.' },
                { icon: 'fa-solid fa-layer-group', title: 'Managed Services', desc: 'End-to-end support for your applications, infrastructure, and digital platforms—so you can shift focus from maintenance to innovation.' },
                { icon: 'fa-solid fa-rocket', title: 'MVP Development', desc: 'Launch market-ready MVPs in 8-12 weeks. AI-accelerated builds with 40% faster iteration cycles.' },
                { icon: 'fa-solid fa-lightbulb', title: 'Technology Consulting', desc: 'Cut tech waste by 30% with our strategic audits. Align tools with business goals for maximum ROI.' },
                { icon: 'fa-solid fa-rotate', title: 'Digital Transformation', desc: 'Modernize legacy systems with AI-driven automation. Achieve 50% operational efficiency gains.' },
                { icon: 'fa-solid fa-microchip', title: 'ML & AI Development', desc: 'Deploy custom neural models, optimize LLM token usage, and train conversational agents to extract deep analytics.' },
              ].map((svc, i) => (
                <a href="#contact" key={i} className="cap-card" style={{ textDecoration: 'none', color: 'inherit' }}>
                  <div className="cap-icon"><i className={svc.icon}></i></div>
                  <h3>{svc.title}</h3>
                  <p>{svc.desc}</p>
                  <span className="cap-link">Learn More <i className="fa-solid fa-arrow-right"></i></span>
                </a>
              ))}
            </div>
          </div>
